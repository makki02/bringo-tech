...
class Image_Size extends Abstract_Template {
	...
	public function apply_filters( $callback ) {
		 ...
		 * Sometimes there are no images for these sizes.
		 *
		 * Do not edit filter hook name definition. It's on purpose!
		 */
		add_filter( 'wp_calculate' . '_image_srcset_meta', [ $this, 'remove_elementor_image_srcset_meta_filter' ], 10 );

		$size   = $this->widget->get_settings_for_display( 'item_size' );
		$result = $callback( $size );

		remove_filter( 'wp_calculate' . '_image_srcset_meta', [ $this, 'remove_elementor_image_srcset_meta_filter' ], 10 );
